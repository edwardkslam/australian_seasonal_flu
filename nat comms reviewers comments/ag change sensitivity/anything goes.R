banana<-unique_list012

banana<-left_join(banana,city_erp)

banana<-banana%>%dplyr::mutate(epi_alarm=ifelse(year_counts>15,"Y","N"))

banana<-banana%>%dplyr::group_by(city,reference_strain,epi_alarm)%>%
...
for(i in 1:nrow(new_ag_problem)){
  banana$new_ag_marker[which(as.character(banana$reference_strain)==as.character(new_ag_problem$reference_strain[i]) &  banana$year==new_ag_problem$year[i])]<-NA
  banana$new_ag_marker[which(as.character(banana$reference_strain)==as.character(new_ag_problem$reference_strain[i]) &  banana$year>new_ag_problem$year[i])]<-0
}


banana<-banana%>%
  dplyr::mutate(log_incidence = log(year_counts/ERP*10^6))
...
banana%>%
  ...
  scale_x_discrete(labels=c("0"="Detected \n Before",
                            "1"="First \n Detected"))+
  #scale_y_continuous(breaks=)+
  ylab("Lab confirmed counts") +
  labs(x=NULL)+
  theme_bw()+
...
banana<-banana%>%dplyr::mutate(incidence_per_mil = year_counts/ERP*10^6)
dodgy_prev<-banana%>%subset(.,is.na(new_ag_marker) | 
...
cumulative_incidence_by_ag<-left_join(cumulative_incidence_by_ag,banana[,c("city","subtype","reference_strain","year","epi_alarm","incidence_per_mil")])
...
prob_successful_epi_cumulative_size_same_variant_plot<-cumulative_incidence_by_ag %>%
  subset(.,!(reference_strain%in%dodgy_prev$reference_strain | subtype=="H1pdm09"))%>%
