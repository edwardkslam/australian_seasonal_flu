...
# 1)  The relationship between epidemic incidence and the amount of antigenic variant-specific cumulative incidence (Figure 4)
#
# 2)  The relationship between the probability of successful epidemic initiation 
#     and amount of antigenic variant-specific cumulative incidence for each subtype (Figure S16)
#
# 3)  Binary logistics regression assessing the effect of antigenic variant-specific cumulative incidence 
#     on the probability of successful epidemic initiation for each subtype (Table S6)
...
cumulative_incidence_by_ag<-cumulative_incidence_by_ag%>%
  left_join(data.frame(city=epi_table$city,reference_strain=epi_table$reference_strain,year=epi_table$year,epi_alarm=factor(epi_table$epi_alarm,levels = c("Y","N"))))
